...
class UiInteractionImpl(
    ...
    override fun runAlarm(status: String, title: String) {
        ...
        // every alarm with the true trigger time, regardless of whether or how it is later
        // acknowledged.
        if (config.APS)
            appScope.launch {
        ...
        if (isAppInForeground()) {
            // Foreground path — launch the activity directly. No notification is needed because
            // the caller's process is already in the foreground.
            ...
            // Background path — FSI notification. Android auto-launches the activity on the lock
            // screen or shows a silent heads-up notification when the user is in another app.

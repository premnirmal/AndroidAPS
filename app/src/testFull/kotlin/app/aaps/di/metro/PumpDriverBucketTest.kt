...
class PumpDriverBucketTest {
    ...
    fun `the pump bucket holds exactly the known drivers`() {
        assertThat(testRoot().contributedPumpDriverPlugins.keys)
            .containsExactly(1010, 1020, 1030, 1040, 1050, 1060, 1070, 1080, 1085, 1090, 1100, 1110, 1120, 1130, 1190)
    }
    ...
    fun `no pump driver leaks into the every-build bucket`() {
        ...
        assertThat(everyBuild).containsNoneOf(1010, 1020, 1030, 1040, 1050, 1060, 1070, 1080, 1085, 1090, 1100, 1110, 1120, 1130, 1190)
    }
}
